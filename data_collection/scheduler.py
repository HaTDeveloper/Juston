"""
Saudi Stock Data Collection Scheduler
------------------------------------
This module handles scheduling periodic updates of stock data.
...
class StockDataScheduler:
    """Class to handle scheduling of stock data collection"""
        ...
        try:
            ...
            def job_wrapper():
                try:
                    ...
                    logger.info(f"Job {job_name} completed in {execution_time:.2f} seconds")
                    return result
            ...
            if interval_minutes < 60:
                ...
                logger.info(f"Scheduled job {job_name} to run every {interval_minutes} minutes")
            else:
                # For longer intervals, schedule at specific times
                hours = interval_minutes // 60
                schedule.every(hours).hours.do(job_wrapper)
                logger.info(f"Scheduled job {job_name} to run every {hours} hours")
...
if __name__ == "__main__":
    ...
    # Initialize the collector and scheduler
    collector = StockDataCollector()
    scheduler = StockDataScheduler()
    
    # Define update function
    def update_all_stocks():
        symbols = collector.get_saudi_market_symbols()[:5]  # Just update first 5 for testing
        return collector.update_stock_data(symbols)
    
    # Schedule the update job
    scheduler.schedule_job(update_all_stocks, interval_minutes=30, job_name="update_stocks")
